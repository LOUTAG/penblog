...
const Categories=({categories, display, setDisplay, cat, searchByCategoryAction})=>{
    //helpers
      ...
      let key =
        ...
        if(index===0) return (
          <React.Fragment key={key}>
            <li
          className={`${cat===null&&"bg-dark-primary"} text-xl rounded-lg font-semibold px-2 py-3 mr-1 capitalize cursor-pointer hover:bg-black-primary hover:shadow`}
          onClick={()=>onCategoryClick(null, null)}
        >
          All
        </li>
        <li
          className={`${category._id===cat?.id&&"bg-dark-primary"} text-xl rounded-lg font-semibold px-2 py-3 mr-1 capitalize cursor-pointer hover:bg-black-primary hover:shadow`}
